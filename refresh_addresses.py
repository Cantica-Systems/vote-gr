...
UA = {"User-Agent": "vote-gr/1.0 (+https://votegr.cantica.dev)"}
PAGE = 2000
DELAY_SECONDS = 2              # be a polite guest on someone else's server

HERE = pathlib.Path(__file__).resolve().parent
BOUNDARIES = HERE / "site" / "data" / "precincts.geojson"
OUT = HERE / "site" / "data" / "addresses.json"
...
def split_address(text):
    """'1951 LAFAYETTE AVE SE' -> (1951, 'LAFAYETTE AVE SE').
